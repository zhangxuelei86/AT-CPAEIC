		...
		real HCde(233),COde(233),NOxde(233),PMde(233),SO2de(233),CO2de(233)	

		CHARACTER(LEN=50),PARAMETER :: departfile='../../output/tmp/out_departdata.csv'
		CHARACTER(LEN=50),PARAMETER :: pdfile='../../input/pd.csv'
		CHARACTER(LEN=50),PARAMETER :: departoutfile='../../output/tmp/demis.csv'


		!20储存数组大小，21储存出发输入值,22机场三字码，24输出
		!open(20,file=)
		open(21,file=departfile)
		open(22,file=pdfile)
		open(23,file=departoutfile)
		!open(24,file='bug.csv')
		...
		TCO2e(i)=0.0
		DO w=1,S
		If(airportcode(w)==airportB(i))THCa(i)=THCa(i)+EHCb(w)
		If(airportcode(w)==airportB(i))THCb(i)=THCb(i)+EHCc(w) 
		If(airportcode(w)==airportB(i))THCc(i)=THCc(i)+EHCd(w)
		If(airportcode(w)==airportB(i))THCd(i)=THCd(i)+EHCg(w)
		If(airportcode(w)==airportB(i))THCe(i)=THCe(i)+EHCh(w)
		  
		If(airportcode(w)==airportB(i))TCOa(i)=TCOa(i)+ECOb(w)
		If(airportcode(w)==airportB(i))TCOb(i)=TCOb(i)+ECOc(w) 
		If(airportcode(w)==airportB(i))TCOc(i)=TCOc(i)+ECOd(w)
		If(airportcode(w)==airportB(i))TCOd(i)=TCOd(i)+ECOg(w)
		If(airportcode(w)==airportB(i))TCOe(i)=TCOe(i)+ECOh(w)


		If(airportcode(w)==airportB(i))TNOxa(i)=TNOxa(i)+ENOxb(w)
		If(airportcode(w)==airportB(i))TNOxb(i)=TNOxb(i)+ENOxc(w) 
		If(airportcode(w)==airportB(i))TNOxc(i)=TNOxc(i)+ENOxd(w)
		If(airportcode(w)==airportB(i))TNOxd(i)=TNOxd(i)+ENOxg(w)
		If(airportcode(w)==airportB(i))TNOxe(i)=TNOxe(i)+ENOxh(w)

		If(airportcode(w)==airportB(i))TBCa(i)=TBCa(i)+EBCb(w)
		If(airportcode(w)==airportB(i))TBCb(i)=TBCb(i)+EBCc(w) 
		If(airportcode(w)==airportB(i))TBCc(i)=TBCc(i)+EBCd(w)
		If(airportcode(w)==airportB(i))TBCd(i)=TBCd(i)+EBCg(w)
		If(airportcode(w)==airportB(i))TBCe(i)=TBCe(i)+EBCh(w)

		If(airportcode(w)==airportB(i))TOCa(i)=TOCa(i)+EOCb(w)
		If(airportcode(w)==airportB(i))TOCb(i)=TOCb(i)+EOCc(w) 
		If(airportcode(w)==airportB(i))TOCc(i)=TOCc(i)+EOCd(w)
		If(airportcode(w)==airportB(i))TOCd(i)=TOCd(i)+EOCg(w)
		If(airportcode(w)==airportB(i))TOCe(i)=TOCe(i)+EOCh(w)

		If(airportcode(w)==airportB(i))TSO4a(i)=TSO4a(i)+ESO4b(w)
		If(airportcode(w)==airportB(i))TSO4b(i)=TSO4b(i)+ESO4c(w) 
		If(airportcode(w)==airportB(i))TSO4c(i)=TSO4c(i)+ESO4d(w)
		If(airportcode(w)==airportB(i))TSO4d(i)=TSO4d(i)+ESO4g(w)
		If(airportcode(w)==airportB(i))TSO4e(i)=TSO4e(i)+ESO4h(w)

		If(airportcode(w)==airportB(i))TPMa(i)=TPMa(i)+EPMb(w)
		If(airportcode(w)==airportB(i))TPMb(i)=TPMb(i)+EPMc(w) 
		If(airportcode(w)==airportB(i))TPMc(i)=TPMc(i)+EPMd(w)
		If(airportcode(w)==airportB(i))TPMd(i)=TPMd(i)+EPMg(w)
		If(airportcode(w)==airportB(i))TPMe(i)=TPMe(i)+EPMh(w)

		If(airportcode(w)==airportB(i))TSO2a(i)=TSO2a(i)+ESO2b(w)
		If(airportcode(w)==airportB(i))TSO2b(i)=TSO2b(i)+ESO2c(w) 
		If(airportcode(w)==airportB(i))TSO2c(i)=TSO2c(i)+ESO2d(w)
		If(airportcode(w)==airportB(i))TSO2d(i)=TSO2d(i)+ESO2g(w)
		If(airportcode(w)==airportB(i))TSO2e(i)=TSO2e(i)+ESO2h(w)

		If(airportcode(w)==airportB(i))TCO2a(i)=TCO2a(i)+ECO2b(w)
		If(airportcode(w)==airportB(i))TCO2b(i)=TCO2b(i)+ECO2c(w) 
		If(airportcode(w)==airportB(i))TCO2c(i)=TCO2c(i)+ECO2d(w)
		If(airportcode(w)==airportB(i))TCO2d(i)=TCO2d(i)+ECO2g(w)
		If(airportcode(w)==airportB(i))TCO2e(i)=TCO2e(i)+ECO2h(w)

		END DO
		END DO
		write(23,'(A)')trim(line1)
		!real HCde(233),COde(233),NOxde(233),PMde(233),SO2de(233),CO2(233)		
		Do w=1,233
		HCde(w)=THCa(w)+THCb(w)+THCc(w)+THCd(w)+THCe(w)
		COde(w)=TCOa(w)+TCOb(w)+TCOc(w)+TCOd(w)+TCOe(w)
		NOxde(w)=TNOxa(w)+TNOxb(w)+TNOxc(w)+ TNOxd(w)+TNOxe(w)
		PMde(w)=TPMa(w)+TPMb(w)+TPMc(w)+TPMd(w)+TPMe(w)
		SO2de(w)=TSO2a(w)+TSO2b(w)+TSO2c(w)+TSO2d(w)+TSO2e(w)
		CO2de(w)=TCO2a(w)+TCO2b(w)+ TCO2c(w)+TCO2d(w)+TCO2e(w)	
		END Do

		Do w=1,233
		write(23,200)airportB(w),HCde(w),COde(w),NOxde(w), &
		& PMde(w),SO2de(w),CO2de(w)
		End Do   
		...
		!Do w=1,233
		! write(23,200)airportB(w),THCa(w),THCb(w),THCc(w), &
		!       & THCd(w),THCe(w),TCOa(w),TCOb(w),TCOc(w), & 
		!       & TCOd(w),TCOe(w),TNOxa(w),TNOxb(w),TNOxc(w), & 
		!        & TNOxd(w),TNOxe(w),TPMa(w),TPMb(w),TPMc(w), & 
		!        & TPMd(w),TPMe(w),TSO2a(w),TSO2b(w),TSO2c(w), & 
		!        & TSO2d(w),TSO2e(w),TCO2a(w),TCO2b(w), & 
		!        & TCO2c(w),TCO2d(w),TCO2e(w)
		!End do
		! 200 format(1x,A6,1x,5F23.6,1x,5F23.6,1x,5F25.3,1x,5F22.4, &
		!     &1x,5F22.3,1x,5F27.3) 
		!200 format(A6,',',F23.6,',',F23.6,',',F23.6,',',F23.6,& 
		!  &',',F23.6,',',F23.6,',',F23.6,',',F23.6,',',F23.6,&
		! &',',F23.6,',',F25.3,',',F25.3,',',F25.3,',',F25.3,&
		! &',',F25.3,',',F22.4,',',F22.4,',',F22.4,',',F22.4,&
		! &',',F22.4,',',F22.3,',',F22.3,',',F22.3,',',F22.3,&
		!  &',',F22.3,',',F27.3,',',F27.3,',',F27.3,','F27.3,&
		! &',',F27.3)
